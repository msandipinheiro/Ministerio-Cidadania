...
# =============================================================================
    def train(self, training_data): # algorith=viterbi
        np.seterr(all='ignore')
        
        cur_model = self.model.get_params(True)
...
if __name__=='__main__':
    ...
    file_test = '/home/marcello/Documentos/work/Ministerio-Cidadania/01-transcritor/data_test/'
    speech_models = build_models(file_test)
    
    sav_model = 'transcritor.sav'
    with open('transcritor.sav', 'wb') as file: 
        #pickle.dump(speech_models, open(sav_model, 'wb'))
        joblib.dump(speech_models, open(sav_model, 'wb'))
    
    with open('transcritor.sav', 'rb') as file: 
        #load_model = pickle.load(open(sav_model, 'rb'))
        load_model = joblib.load(open(sav_model, 'rb'))
    
    #file_test = '/home/marcello/Documentos/work/Ministerio-Cidadania/01-transcritor/ministerio-da-cidadania.wav'
    sampling_freq, signal = wavfile.read(file_test)
    features_mfcc = mfcc(signal, sampling_freq, nfft=1200)
    #r_score       = load_model.compute_score(features_mfcc)
    #r_predic_prob = load_model.compute_predict(features_mfcc)
    r_score       = load_model(features_mfcc)
    print('score: ')
    print(r_score)
    #print('predic_prob: ')
    #print(r_predic_prob)
